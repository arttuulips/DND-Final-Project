...
public class BlogPostLogic : IBlogPostLogic
{
    ...
    public async Task<BlogPost> CreateAsync(BlogPostCreationDto dto, string userId)
    {
        User? user = await userDao.GetByIdAsync(int.Parse(userId));
        if (user == null)
        {
        ...
        BlogPost blogPost = new BlogPost(user.Id, dto.Title, dto.Content, dto.Country);
    ...
    public async Task UpdateAsync(BlogPostUpdateDto dto)
    {
        ...
        BlogPost updated = new (userToUse.Id, titleToUse, contentToUse, countryToUse)
        {
    ...
    public async Task<BlogPostBasicDto> GetByIdAsync(int id)
    {
        ...
        return new BlogPostBasicDto(blogPost.Id, blogPost.Author.UserName, blogPost.Title, blogPost.Content, blogPost.Country,blogPost.IsPublished);
    }
